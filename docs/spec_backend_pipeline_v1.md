...
| ⬜ | T32 | Investigate `run_category`'s all-or-nothing rollback behavior in `scripts/discover_products.py`: when `CoolblueBlockedError` is re-raised after `MAX_CONSECUTIVE_403S` consecutive blocks, `conn.rollback()` discards every successfully-scraped product from that entire run, not just the ones affected by blocking. Surfaced while implementing the embeddings spec's discovery hook (docs/spec_embeddings.md R-8) — not fixed there to avoid changing this script's commit/rollback structure as part of an unrelated feature. Determine whether discarding all prior successful work on a late-run block is actually the desired behavior, or whether products processed before blocking started should be preserved. | 12 |
| ⬜ | T33 | Fix `scrape_runs` entries being left permanently "in progress" when a script is interrupted (Ctrl+C / `KeyboardInterrupt`) mid-run: every script following the `run_id`/`finish_scrape_run` pattern (`discover_products.py`, `scrape_price_history.py`, `backfill_ai_descriptions.py`, `backfill_ai_deal_descriptions.py`, `backfill_embeddings.py`) only wraps `except Exception`, which does not catch `KeyboardInterrupt` (it's a `BaseException`, not an `Exception`) — the `finally` block still runs, but `finish_scrape_run` is only called from inside the `except Exception` branch, so it's skipped entirely on interrupt. Surfaced during `backfill_embeddings.py` verify (T_E4). Fix once, consistently, across all affected scripts rather than patching one at a time. | 12 |
