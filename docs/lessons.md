...
- **A staleness question caught before coding beats one caught after.** Raising "embeddings never regenerate, but price changes daily" before T_E3 started avoided writing `get_price_tier`, its tests, and threading `current_price` through two queries — only to rip it all out later. The earlier a drift/staleness concern surfaces for any "write once, never refresh" field, the cheaper the fix.
- **A design decision made here rippled into a second repo's spec.** Excluding price from the embedding wasn't just a `product_scraper` change — it invalidated a "what to embed" row in `price-tracker-web`'s spec and exposed an undesigned gap (no price/deal-aware ranking). Cross-repo consistency checks are worth doing explicitly whenever a decision changes a data contract another repo depends on, not just assuming the other side will notice.
- **Matching an established codebase convention is a legitimate verify-pass finding, not just a nitpick.** `ai_descriptions.py` catches broad `Exception`; the first `embeddings.py` draft narrowly caught `openai.OpenAIError`, technically satisfying the spec's own test wording but diverging from precedent. Comparing new code against the nearest sibling module during verify — not just against the spec — catches this kind of drift.
- **CLAUDE.md's "don't validate scenarios that can't happen" cuts both ways.** It correctly ruled out guarding `build_embedding_text` against a `None` product (no real call site does that) but shouldn't be used to wave off genuine external-API-boundary hardening (broadening the exception catch) — the rule is about internal contracts, not the API boundary, and it's worth pausing to check which category a given finding falls into before applying it.

---

## T_E5 — Add embedding discovery hook to discover_products.py

- **Diff new DB-write functions against sibling *transaction* behavior, not just SQL shape.** `store_embedding`'s internal `conn.commit()` was spec-compliant on paper but silently broke `discover_products.py`'s all-or-nothing rollback, because its closest precedent (`update_ai_description`) doesn't commit — the caller does. Comparing error-handling scope against precedent (T_E3's lesson) generalizes to comparing commit/transaction behavior too.
...
## T38 — Review and improve AI prompt wording (product + deal descriptions)

- **Sampling real stored production output caught more than a fresh live preview would have.** The original T29 preview only ran fresh one-off generations. Sampling actual prod rows this time surfaced concrete failure instances a handful of live test calls likely wouldn't hit — the invented "vorige week" timeframe, and "laagste prijs ooit" being unverifiable against the site's 90-day graph. When revisiting an existing AI feature, prefer sampling real stored output over generating fresh previews.
- **Moving subjective judgment out of the prompt and into deterministic code beats better wording.** Asking the model to "pick the most salient fact" produced inconsistent phrasing across similar inputs. Replacing it with a small Python classifier (`_classify_price_situation`) that hands the model exactly one pre-chosen fact removed the ambiguity entirely, at effectively zero extra cost. Reach for this whenever a prompt asks the model to judge rather than just phrase given facts.
- **`max_tokens` is a truncation ceiling, not a shortening lever — confirmed the hard way** (a live response cut off mid-sentence at `max_tokens=60`). Length control belongs in the prompt's structure/instructions; `max_tokens` should carry headroom as a safety backstop only.
- **User domain knowledge caught a correctness bug that prompt sampling alone never would have.** "Laagste prijs ooit" was technically true but practically unverifiable — the site only displays 90 days of price history, and prod data already spans 175 days. Worth explicitly asking "does this claim match what the user can actually verify downstream?" for any AI-generated text referencing external context.
...
## T39 — Cost-cutting for AI generation

- **Real diagnostic data overturned the spec's own cost assumptions.** The documented estimate was "~20–50 deal-description calls/day," but the actual measured volume was ~115/day (2–5x higher, already at what the spec called "sales-event" levels — every day, not occasionally). Querying live data before optimizing, rather than trusting a spec estimate written months earlier, changed the whole scope of what was worth fixing.
- **A deterministic-classification decision made for one problem (T38's quality fix) quietly solved a different problem later (T39's cost).** Recognizing that `_classify_price_situation`'s "none" bucket meant "the model is just restating a pre-computed fact" made that bucket template-eligible for free — and the same classification logic, replicated in SQL, let the cost/benefit of templating be measured before writing any Python.
- **Templating-that-still-writes beats skipping.** Reframing "skip regeneration below a threshold" as "replace the API call with a cheap deterministic equivalent" avoided a stale/missing-data risk *and* a threshold-policy debate (what %, absolute € vs. percentage) — collapsing two of the spec's five candidate options into one better implementation.
- **User pushback on "cheapest possible" surfaced a real trade-off a pure cost-minimization plan would have flattened.** Extending templating to all 4 buckets would have hit ~$0/year, but at the cost of losing the richer phrasing for the genuinely newsworthy cases — worth asking "what does this optimization actually give up" rather than just chasing the lowest number.