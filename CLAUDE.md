...
- Before treating a diagnostic SQL result as authoritative (e.g. checking a Postgres version or extension availability), confirm which server/connection it ran against — especially in pgAdmin with both local and Railway connections open side by side. A query run against the wrong connection looks identical to a real anomaly.
- When multiple native Postgres versions coexist on one Windows machine, `pg_config`/tools on PATH may resolve to a different version than the one actually running the dev DB. Check the target service name (e.g. `postgresql-x64-18`) before assuming PATH points at the right install, and set `PGROOT` explicitly for any version-specific build/install step (e.g. building pgvector from source).
- Before a script's first costly/production run, if it supports `--dry-run`, run the dry-run at full scale (no `--limit`) first — it's free and catches encoding/format edge cases across the whole dataset that a small `--limit` sample misses (e.g. `backfill_embeddings.py`'s T_E4 dry-run caught an accented-character display bug across 698 rows that a 5-row sample didn't hit)
...
- Scripts with emoji/unicode in `print()` statements (`scripts/discover_products.py`, `scripts/scrape_price_history.py`) call `sys.stdout.reconfigure(encoding="utf-8")` / `sys.stderr.reconfigure(...)` near the top, guarded by `try/except AttributeError` — default Windows console encoding (cp1252) crashes mid-print on emoji, which can escape exception handlers and zero out `scrape_runs` counters before they're written. Any new CLI script with emoji output needs the same guard.
